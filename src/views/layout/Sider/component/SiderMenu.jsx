...
renderSubMenu = ({ name, title, icon, children }) => {
  ...
  return (
    ...
    >
      {children && children.map(item => (item.children && item.children.filter(s => s.leftMenu).length > 0
...
render() {
  ...
  return (
    <Sider
      ...
      width={180}
    >
      <div className="logo" style={{ paddingLeft: this.props.collapsed ? '18px' : '6px' }}><img src={logo} alt="logo" /><span style={{display: this.props.collapsed ? 'none' : 'block'}}>AU-andmin</span></div>
      <Menu
        mode="inline"
      ...
      >
        {menus
      && menus.map(item => (item.children && item.children.length
        ? this.renderSubMenu(item)
        : this.renderMenuItem(item)))}
      </Menu>
    </Sider>
