...
const SchemaUtils = {
  ...
  createForm(id, schema, uiSchema, noCache) {
    ...
    const tmpComponent = createClass({
      componentWillMount() {
        ...
        // !s && JsxGeneratorMap.set(id, generateJsx);
  ...
  parse(schema, uiSchema) {
    ...
    Object.keys(uiSchema).forEach((key) => {
      ...
      field.key = key; // getFieldDecorator/formItem输入控件唯一标志
      const schemaProperty = schemaProperties[key];
      // console.log('uiSchema/[key]', field);
      // console.log('schema/Properties[key]', schemaProperty);
